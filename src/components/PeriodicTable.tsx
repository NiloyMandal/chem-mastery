...
import {
  ...
  CATEGORIES,
  Element,
  getElectronegativityRange,
  getAtomicRadiusRange,
...
export default function PeriodicTable() {
  ...
  const getHeatmapStyle = (el: Element): React.CSSProperties => {
    ...
    if (viewMode === "electronegativity" && el.electronegativity) {
      const norm =
        (el.electronegativity - enRange.min) / (enRange.max - enRange.min);
      const hue = 240 - norm * 240; // Blue to Red
      return {
        backgroundColor: `hsl(${hue}, 85%, 75%)`,
        borderColor: `hsl(${hue}, 85%, 50%)`,
      };
    }
    if (viewMode === "atomic-radius" && el.atomicRadius) {
      const norm =
        (el.atomicRadius - arRange.min) / (arRange.max - arRange.min);
        ...
        <div className="flex flex-col md:flex-row flex-wrap gap-3 w-full xl:w-auto">
          ...
          {viewMode === "state" && (
            <div className="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-xl">
              ...
              />
              <span className="text-sm font-mono font-bold text-slate-700 w-16 text-right">
                {temperature} K
              </span>
            </div>
          )}
        ...
        {viewMode === "categories" ? (
          <div className="flex gap-1.5 flex-wrap">
            {Object.keys(CATEGORIES).map((cat) => {
              ...
              const colorClass =
                CATEGORY_COLORS[catKey] || "bg-slate-100 border-slate-200";
              // Extract just the background color for the dot
              const bgOnly =
                colorClass.split(" ").find((c) => c.startsWith("bg-")) ||
                "bg-slate-200";
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    categoryFilter === cat
                      ? colorClass +
                          " ring-1 ring-offset-1 ring-slate-400 scale-105"
            ...
            {categoryFilter && (
              <button
                ...
                className="px-3 py-1 rounded-full text-xs font-medium bg-slate-800 text-white hover:bg-slate-900">
                Reset
              </button>
            )}
          </div>
        ...
        {viewMode === "state" && (
          <div className="flex items-center gap-3 px-3 py-1.5 bg-white rounded-lg border border-slate-100 shadow-sm text-xs">
            <span className="flex items-center gap-1">
              <Snowflake className="w-3 h-3 text-blue-500" /> Solid
            </span>
            <span className="flex items-center gap-1">
              <Droplet className="w-3 h-3 text-cyan-500" /> Liquid
            </span>
            <span className="flex items-center gap-1">
              <Cloud className="w-3 h-3 text-purple-500" /> Gas
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 gap-4 min-h-0 relative">
        ...
        <div className="flex-1 overflow-auto pr-1 pb-4">
          <div className="grid grid-cols-[repeat(18,minmax(1.8rem,1fr))] gap-1 auto-rows-[2rem] md:auto-rows-[2.4rem]">
                ...
                <motion.button
                  ...
                  className={cn(
                    ...
                    selected?.number === el.number &&
                      "ring-2 ring-indigo-600 z-20 scale-110 shadow-xl",
                    hovered?.number === el.number &&
                      !selected &&
                      "z-10 scale-110 shadow-lg brightness-95",
                  )}
                  whileTap={{ scale: 0.95 }}>
                  <span className="text-[0.5rem] md:text-[0.6rem] font-mono absolute top-0.5 left-1 opacity-60">
                    {el.number}
                  </span>
            ...
            <div className="col-start-3 row-start-6 border border-dashed border-slate-300 rounded-md flex items-center justify-center text-[0.6rem] text-slate-400">
              57-71
            </div>
            <div className="col-start-3 row-start-7 border border-dashed border-slate-300 rounded-md flex items-center justify-center text-[0.6rem] text-slate-400">
              89-103
            </div>
          </div>
        </div>
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
                ...
                <div className="flex items-center gap-6">
                  <div
                    ...
                    <span className="text-xs md:text-sm font-mono mt-1 font-semibold opacity-70">
                  ...
                  <div>
                    ...
                    <p className="text-lg text-slate-500 font-mono mb-1">
                      {activeElement.mass.toFixed(4)} u
                    </p>
                ...
                <div className="flex-1 w-full grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 col-span-2">
                    <p className="text-[0.65rem] uppercase tracking-wider text-slate-400 font-bold mb-1">
                      Electron Config
                    </p>
                  ...
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 col-span-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      activeElement.oxidationStates.length > 0 ? (
                  ...
                  <PropCard
                    ...
                    value={activeElement.electronegativity || "-"}
                  />
                  <PropCard
                    label="Radius"
                    value={
                      activeElement.atomicRadius
                        ? `${activeElement.atomicRadius} pm`
                        : "-"
                    }
                  />
                  <PropCard
                    label="Density"
                    value={
                      activeElement.density
                        ? `${activeElement.density} g/cm³`
                        : "-"
                    }
                  />
                  <PropCard
                    label="Melting Point"
                    value={
                      activeElement.meltingPoint
                        ? `${activeElement.meltingPoint} K`
                        : "-"
                    }
                  />
                  <PropCard
                    label="Boiling Point"
                    value={
                      activeElement.boilingPoint
                        ? `${activeElement.boilingPoint} K`
                        : "-"
                    }
                  />
                  {activeElement.yearDiscovered && (
                    <PropCard
                      label="Discovered"
                      value={activeElement.yearDiscovered}
                    />
                  )}
                </div>
              </div>
...
function PropCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-white border border-slate-100 p-2.5 rounded-xl shadow-sm">
      <p className="text-[0.6rem] text-slate-400 uppercase font-bold">
        {label}
      </p>
