...
export default function Thermometer({
    ...
    return (
        <div className={cn("relative flex flex-col items-center", className)}>
            ...
            <div className="w-4 h-48 bg-slate-200/50 rounded-t-full border border-slate-300 relative overflow-hidden backdrop-blur-sm z-10">
                ...
                <div className="absolute right-0 top-0 bottom-0 w-full flex flex-col justify-between py-2 px-0.5 pointer-events-none z-20">
                    {[max, max * 0.75, max * 0.5, max * 0.25, min].map((_val, i) => (
                        <div key={i} className="w-1.5 h-[1px] bg-slate-400 self-end opacity-50" />
                    ))}
                </div>

                {/* The Mercury (Liquid) */}
                <motion.div
                    className={cn("w-full absolute bottom-0 transition-colors duration-500", getColor(temperature))}
                    initial={{ height: '25%' }}
            ...
            {/* Label */}
            <p className="text-[0.6rem] text-slate-500 font-medium mt-2 uppercase tracking-wider">Temp</p>
        </div>
    );
}
