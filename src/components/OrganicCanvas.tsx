...
export default function OrganicCanvas() {
  ...
  // ── Synchronous Derived Values (no useEffect lag) ──────────────────────

  const getSnapPos = useCallback(
    (mouse: Point, start: Point): Point => {
      const dx = mouse.x - start.x;
      const dy = mouse.y - start.y;
      const angle = Math.atan2(dy, dx);
      const snapAngle = Math.round(angle / (Math.PI / 6)) * (Math.PI / 6);
      const raw: Point = {
        x: start.x + Math.cos(snapAngle) * BOND_LENGTH,
        y: start.y + Math.sin(snapAngle) * BOND_LENGTH,
      };
      return findNearestVertex(raw, state.bonds, SNAP_THRESHOLD) ?? raw;
    },
    [state.bonds],
  );

  // Derived: current snap position for bond preview
  const snappedPos = useMemo(() => {
    if (!currentStart) return null;
    return getSnapPos(mousePos, currentStart);
  }, [mousePos, currentStart, getSnapPos]);

  // Derived: hovered bond/label indices for eraser highlighting
  const { hoveredBondIdx, hoveredLabelIdx } = useMemo(() => {
    if (tool !== "eraser") return { hoveredBondIdx: -1, hoveredLabelIdx: -1 };
    ...
    return { hoveredBondIdx: closestBond, hoveredLabelIdx: closestLabel };
  }, [mousePos, tool, state.bonds, state.atomLabels]);

  // Derived: nearest vertex indicator (when not drawing)
  const nearestVertex = useMemo(() => {
    if (tool !== "bond" || currentStart) return null;
    return findNearestVertex(mousePos, state.bonds, SNAP_THRESHOLD);
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    // 1. If we were panning, just release the pan — do NOT draw
    ...
    // 2. Only left-click triggers drawing
    if (e.button !== 0) return;

    // 3. Original drawing logic (previously in handleClick)
    const coords = getSvgCoordinates(e);

    if (tool === "eraser") {
      // Compute hovered targets inline for frame-perfect accuracy
      let eraseBondIdx = -1;
      let eraseBondDist = SNAP_THRESHOLD;
      state.bonds.forEach((bond, idx) => {
        const d = pointToSegmentDist(coords, bond.start, bond.end);
        if (d < eraseBondDist) {
          eraseBondDist = d;
          eraseBondIdx = idx;
        }
      ...
      let eraseLabelIdx = -1;
      let eraseLabelDist = SNAP_THRESHOLD;
      state.atomLabels.forEach((label, idx) => {
        const d = Math.hypot(
          label.position.x - coords.x,
          label.position.y - coords.y,
        );
        if (d < eraseLabelDist) {
          eraseLabelDist = d;
          eraseLabelIdx = idx;
      ...
      if (eraseLabelIdx >= 0) {
        newLabels = state.atomLabels.filter((_, i) => i !== eraseLabelIdx);
        changed = true;
      } else if (eraseBondIdx >= 0) {
        newBonds = state.bonds.filter((_, i) => i !== eraseBondIdx);
        changed = true;
      }
      if (changed) {
    ...
    if (tool === "label") {
      ...
      pushState({
        ...
        atomLabels: [
          ...state.atomLabels,
          { position: pos, label: selectedLabel },
        ],
      });
      return;
    }

    // Bond tool — compute snap inline for frame-perfect accuracy
    if (!currentStart) {
      const vertex = findNearestVertex(coords, state.bonds, SNAP_THRESHOLD);
      setCurrentStart(vertex ?? coords);
    } else {
      const snap = getSnapPos(coords, currentStart);
      pushState({
        ...state,
        bonds: [
          ...state.bonds,
          { start: currentStart, end: snap, type: bondType },
        ],
      });
      setCurrentStart(snap);
    }
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isPanning && panStart) {
      const dx =
        ...
        (viewBox.width / (containerRef.current?.clientWidth ?? 800));
      const dy =
        (e.clientY - panStart.y) *
        (viewBox.height / (containerRef.current?.clientHeight ?? 600));
      setViewBox((prev) => ({
        ...prev,
        x: panStart.vx - dx,
        y: panStart.vy - dy,
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    setViewBox((prev) => {
      ...
      const newH = Math.min(
        MAX_ZOOM * 0.75,
        Math.max(MIN_ZOOM * 0.75, prev.height * factor),
      );
    ...
    for (let i = 0; i < 6; i++) {
      ...
      newBonds.push({
        start: {
          ...
          x: centerX + Math.cos(a2) * radius,
          y: centerY + Math.sin(a2) * radius,
        },
  ...
  const exportSvg = () => {
    ...
    // 1. Remove preview elements AND the background grid
    clone.querySelectorAll("[data-preview]").forEach((el) => el.remove());
    clone.querySelector("rect[fill='url(#dotGrid)']")?.remove();
    clone.querySelector("defs")?.remove();

    // 2. Compute bounding box around all content so nothing gets cropped
    const padding = 20;
    let minX = Infinity,
      minY = Infinity,
      maxX = -Infinity,
    ...
    for (const label of state.atomLabels) {
    ...
    // Fall back to current viewBox if canvas is empty
    if (!isFinite(minX)) {
      minX = viewBox.x;
      minY = viewBox.y;
    ...
    const exportVB = `${minX - padding} ${minY - padding} ${maxX - minX + padding * 2} ${maxY - minY + padding * 2}`;
    ...
    bg.setAttribute("width", String(maxX - minX + padding * 2));
    bg.setAttribute("height", String(maxY - minY + padding * 2));
  ...
  const renderBond = (bond: Bond, idx: number, isPreview = false) => {
    ...
    if (bond.type === "double") {
      ...
      return (
        <g key={idx}>
          <line
            x1={bond.start.x + nx * offset}
            y1={bond.start.y + ny * offset}
            x2={bond.end.x + nx * offset}
            y2={bond.end.y + ny * offset}
            strokeWidth="2"
            {...baseProps}
          />
          <line
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex gap-3 items-center flex-wrap">
        ...
        <div className="flex gap-0.5 border-r border-slate-200 pr-3">
          <button
            ...
            className={cn(
              "p-2 rounded-lg transition-all duration-150 border-2",
              tool === "bond"
          ...
          <div className="flex gap-0.5 border-r border-slate-200 pr-3 overflow-x-auto">
            {ATOM_LABELS.map((label) => (
              <button
                ...
                className={cn(
                  "px-2 py-1 rounded-md text-xs font-bold transition-all duration-150",
                  selectedLabel === label
                ...
                style={
                  selectedLabel === label
                    ? { backgroundColor: ATOM_COLORS[label] ?? "#374151" }
                    : undefined
                }>
                {label}
              </button>
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <span className="text-xs font-mono text-slate-400 w-10 text-center tabular-nums">
            {zoomPercent}%
          </span>
          <button
            onClick={zoomOut}
      ...
      <div
        ...
        <svg
          ...
          {currentStart && snappedPos && tool === "bond" && (
            <g data-preview="true">
              {renderBond(
                { start: currentStart, end: snappedPos, type: bondType },
                -1,
                true,
              )}
              <circle
