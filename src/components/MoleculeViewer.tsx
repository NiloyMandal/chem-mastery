...
import { RotateCw, Info, RefreshCcw, Maximize } from "lucide-react";
import {
  MOLECULES,
  ELEMENTS,
  AtomData,
      ...
      {isHovered && meshRef.current && (
        <Html
          position={[
            ...
            meshRef.current.position.y + config.radius + 0.3,
...
function MoleculeRenderer({
  ...
  data: { atoms: AtomData[]; bonds: BondData[] };
  explosion: number;
}) {
...
export default function MoleculeViewer() {
  const [activeMolecule, setActiveMolecule] =
    useState<keyof typeof MOLECULES>("benzene");
